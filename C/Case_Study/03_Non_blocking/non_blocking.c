...
void bcs(grid_type *data)
{
  ...
  MPI_Isend(data->data, 1, type_l_s, x_min_rank, TAG, cart_comm, requests);
  //NB requests+1 == &requests[1]
  MPI_Isend(data->data, 1, type_r_s, x_max_rank, TAG, cart_comm, requests+1);
  MPI_Isend(data->data, 1, type_d_s, y_min_rank, TAG, cart_comm, requests+2);
  MPI_Isend(data->data, 1, type_u_s, y_max_rank, TAG, cart_comm, requests+3);

  MPI_Irecv(data->data, 1, type_l_r, x_min_rank, TAG, cart_comm, requests+4);
  MPI_Irecv(data->data, 1, type_r_r, x_max_rank, TAG, cart_comm, requests+5);
  MPI_Irecv(data->data, 1, type_d_r, y_min_rank, TAG, cart_comm, requests+6);
  MPI_Irecv(data->data, 1, type_u_r, y_max_rank, TAG, cart_comm, requests+7);
