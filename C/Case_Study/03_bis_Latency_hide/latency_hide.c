...
int main(int argc, char ** argv)
{
  ...
  for (icycle=0;icycle<500;++icycle){
    ...
    MPI_Startall(8, requests);
    //loop over only those parts of the array that do not depend on the guard
    //cells
    for (iy=2;iy<=ny_local-1;++iy){
      for (ix=2;ix<=nx_local-1;++ix){
    ...
    MPI_Waitall(4, requests+4, statuses+4);
